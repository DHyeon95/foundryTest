...
describe("SaleContract", function () {
  ...
  describe("Set state", function () {
    ...
    it("should fail set state from other", async function () {
      await expect(saleContract.connect(testUser).setSwitch(true)).to.be.revertedWithCustomError(saleContract, "OwnableUnauthorizedAccount");
      expect(await saleContract.killSwitch()).to.equal(false);

      await expect(saleContract.connect(testUser).setSBTContract(testUser.address)).to.be.revertedWithCustomError(saleContract, "OwnableUnauthorizedAccount");
      expect(await saleContract.tokenContract()).to.equal(constants.ZERO_ADDRESS);
  ...
  describe("Contract connect", function () {
    before(async function () {
      await priceContract.setSBTPrice(200000);
    ...
    it("should mint exact value(USDC)", async function () {
      await usdcContract.connect(assetWallet).transfer(owner.address, 1000000);
      await usdcContract.connect(assetWallet).transfer(testUser.address, 1000000);
    ...
    it("should withdraw from owner", async function () {
      ...
      await expect(await saleContract.withdrawUSDC(500)).to.changeTokenBalances(usdcContract, [owner, saleContract], [500, -500]);
